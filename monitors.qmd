...
average <- data_prepared |>
  ...
  summarize(x = "Inland Empire",
            average_pm25 = mean(`PM2.5 (ug/m3)`),
            average_pm10 = mean(`PM10 (ug/m3)`),
            average_aqi = mean(`AQI US`)) |>
  bind_rows(data.frame(x = c('WHO Guideline', 'US', 'EPA Standard', 'SoCal'),
...
health <- data.frame(condition = c(rep(c("Lung Cancer", "Lung Cancer Mortality", "Pneumonia", "Childhood Asthma"), each = 2)),
                     ...
                     risk = c(100, 33, 100, 18, 100, 15, 100, 16),
                     height = c(NA, 116.5, NA, 109, NA, 107.5, NA, 108),
                     increase = c(NA, "+33%", NA, "+18%", NA, "+15%", NA, "+16%"))
...
# "On a scale of 1 to 10, how would you rate your area's air quality?"
```
...
nov_3_high <- daily_per_monitor |>
  filter(year == 2025 & month == 11 & day == 3) |>
  drop_na()

nov_17_low_rain <- daily_per_monitor |>
  filter(year == 2025 & month == 11 & day == 17) |>
  drop_na()
